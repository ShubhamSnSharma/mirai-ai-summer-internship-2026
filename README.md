# MirAI School of Technology — Virtual Summer Internship 2026

**Student:** Shubham Sharma  
**Track:** AI Builder  
**Stack:** Python · Streamlit · Google Gemini 2.5 Flash · Pandas · Plotly · ReportLab  

---

## Repository Structure

```
mirai-ai-summer-internship-2026/
│
├── Assignment 1/          ← The Identity Echo Interface
├── Assignment 2/          ← Upgrading the AI Multiverse
├── Assignment 3/          ← The Memory Vault (Stateful Chatbot)
├── Assignment 4/          ← Upgrading the AI Image Studio
├── Assignment 5/          ← The Multi-Modal Visual Novel
├── Assignment 6/          ← GitHub Profile: The Hacker Terminal
├── Assignment 7/          ← The Life-OS Wellbeing Dashboard
└── Capstone Project/      ← The AI Resume Critic
```

---

## Assignments

| # | Title | Key Concepts |
|---|-------|-------------|
| [1](Assignment%201/) | The Identity Echo Interface | Streamlit forms, conditional routing, token estimation |
| [2](Assignment%202/) | Upgrading the AI Multiverse | Sidebar UI, persona expansion, chat elements, dynamic avatars |
| [3](Assignment%203/) | The Memory Vault | `st.session_state`, stateful chat, walrus operator |
| [4](Assignment%204/) | Upgrading the AI Image Studio | API URL parameters, file downloads, UX toggles |
| [5](Assignment%205/) | The Multi-Modal Visual Novel | JSON parsing, dynamic UI generation, TTS audio, error handling |
| [6](Assignment%206/) | GitHub Profile: The Hacker Terminal | GitHub special repo, Markdown terminal design, stats cards |
| [7](Assignment%207/) | The Life-OS Wellbeing Dashboard | Pandas, data visualization, AI lifestyle coaching |

---

## Capstone Project — The AI Resume Critic

A production-grade, recruiter-intelligence workspace powered by **Gemini 2.5 Flash**.

**Key Features:**
- Single-call Gemini pipeline returning a 17-section structured JSON contract
- 5-axis visual analytics (Plotly radar chart + skill alignment bars)
- Interactive Resume Studio with 3 live-switching layout templates
- Production export engine (PDF via ReportLab · DOCX via Python-Docx)

```bash
cd "Capstone Project"
pip install -r requirements.txt
streamlit run app.py
```

---

*MirAI School of Technology — Virtual Summer Internship 2026*
